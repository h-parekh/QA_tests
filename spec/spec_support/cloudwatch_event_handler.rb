...
module CloudwatchEventHandler
  def self.set_aws_config
    ...
    Aws.config.update(region: aws_env_config_file.fetch(target_aws_account.to_s).fetch("default_region_name"))
    Aws.config.update(credentials: Aws::Credentials.new(aws_env_config_file.fetch(target_aws_account.to_s).fetch("aws_access_key_id"), aws_env_config_file.fetch(target_aws_account.to_s).fetch("aws_secret_access_key")))
